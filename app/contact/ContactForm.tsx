...
export default function ContactForm() {
  ...
  const formExit = prefersReducedMotion
    ? {}
    : { opacity: 0, y: -20, transition: { duration: 0.3 } }

  const successEnter = prefersReducedMotion
    ? { opacity: 1, y: 0 }
    : { opacity: 0, y: 20 }

  const successAnimate = { opacity: 1, y: 0 }
  ...
  return (
    <AnimatePresence mode="wait">
      {submitted ? (
        <motion.div
          key="success"
          initial={successEnter}
          animate={successAnimate}
          transition={{ duration: 0.4, ease: 'easeOut' }}
          className="bg-brand-surface border border-brand-accent2/30 rounded-2xl p-10 flex flex-col items-center text-center gap-4"
        >
          <div className="w-14 h-14 bg-brand-accent2/20 border border-brand-accent2/30 rounded-2xl flex items-center justify-center">
            {prefersReducedMotion ? (
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" aria-hidden="true">
                <path d="M6 14 L12 20 L22 9" stroke="#00E5A0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            ) : (
              <CheckmarkIcon />
            )}
