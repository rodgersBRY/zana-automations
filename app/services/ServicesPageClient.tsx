    ...
    <div className="pt-24 pb-24 bg-brand-bg min-h-screen">
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.p
            variants={itemVariants}
            className="font-body text-lg text-brand-subtle leading-relaxed max-w-2xl"
          >
            We specialise in four areas where manual work kills the most time
            and costs the most money for Kenyan businesses. Each service is
            built to fit how your business actually operates — not a generic
            template.
          </motion.p>
        </motion.div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <AnimatedHeading
                    as="h2"
                    className="font-display font-bold text-3xl text-brand-text tracking-tight mb-3"
                  >
                    {service.name}
                  </AnimatedHeading>
                  <p className="font-body text-brand-accent2 font-medium mb-4">
                    {service.tagline}
                  </p>
                ...
                <TiltCard className="bg-brand-surface border border-brand-border rounded-2xl p-6 flex flex-col gap-6">
                  <div>
                    <p className="font-body text-xs font-medium text-brand-subtle uppercase tracking-widest mb-3">
                      What we build
                    </p>
                    <ul className="flex flex-col gap-2.5" role="list">
                      {service.examples.map((ex) => (
                        <li key={ex} className="flex items-center gap-3">
                          <span
                            className="w-1.5 h-1.5 bg-brand-accent2 rounded-full shrink-0"
                            aria-hidden="true"
                          />
                          <span className="font-body text-sm text-brand-subtle">
                            {ex}
                          </span>
                        </li>
                      ))}
                  ...
                  <div className="border-t border-brand-border pt-5">
                    ...
                    <div className="flex flex-col gap-2">
                      {service.stats.map((stat) => (
                        <span
                          key={stat}
                          className="font-body text-sm text-brand-accent2 font-medium"
                        >
                          {stat}
                        </span>
                      ))}
