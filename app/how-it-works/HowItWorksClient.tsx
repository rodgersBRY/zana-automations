...
import { pathDraw, stepNode, viewportConfig } from '@/lib/motion'
    ...
    <div className="pt-24 pb-24 bg-brand-bg min-h-screen">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div ref={stepsRef} className="relative mb-20">
          {/* SVG connector — horizontal on desktop, vertical on mobile */}
          <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
            {/* Desktop horizontal line */}
            <svg
              className="hidden md:block absolute top-16 left-0 w-full"
              height="4"
              preserveAspectRatio="none"
            >
              <motion.path
                d="M 60 2 L calc(100% - 60) 2"
                stroke="#1E1E2A"
                strokeWidth="1.5"
                fill="none"
                strokeLinecap="round"
            ...
            <svg
              className="md:hidden absolute left-10 top-0 h-full"
              width="4"
              preserveAspectRatio="none"
            >
              <motion.path
                d="M 2 60 L 2 calc(100% - 60)"
                stroke="#1E1E2A"
                strokeWidth="1.5"
                fill="none"
                strokeLinecap="round"
          ...
          <div className="grid md:grid-cols-3 gap-0 md:gap-8">
            {HOW_IT_WORKS_STEPS.map((step, i) => {
              ...
              const nodeVariants = stepNode(1.2 + i * 0.4)
              return (
                <motion.div
                  key={step.number}
                  ...
                  initial="hidden"
                  animate={prefersReducedMotion || isInView ? 'visible' : 'hidden'}
                  className="grid md:block grid-cols-[80px_1fr] gap-6 py-12 md:py-0 border-t md:border-t-0 border-brand-border md:text-center"
                >
                  <div className="flex flex-col items-start md:items-center gap-2 mb-0 md:mb-6">
                    <div className="relative">
                      <div className="w-20 h-20 md:w-24 md:h-24 bg-brand-surface border border-brand-border rounded-2xl flex items-center justify-center">
                        {Icon && <Icon className="w-7 h-7 md:w-8 md:h-8 text-brand-accent" />}
                      </div>
                  ...
                  <div>
                    <h2 className="font-display font-bold text-xl md:text-2xl text-brand-text tracking-tight mb-3">
                      {step.title}
                    </h2>
                    <p className="font-body text-sm md:text-base text-brand-subtle leading-relaxed">
                      {step.description}
                    </p>
                  </div>
                </motion.div>
        ...
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-8 mb-20">
          ...
          <div className="grid sm:grid-cols-4 gap-6">
            {[
              { period: "Day 1", label: "Discovery call", desc: "Map your processes, agree on scope and timeline" },
              { period: "Week 1", label: "Audit & design", desc: "Detailed workflow design, tools selected, plan confirmed" },
              { period: "Week 2–3", label: "Build & test", desc: "System built, tested against real scenarios, refined" },
              { period: "Week 4", label: "Launch & train", desc: "Go live, team trained, documentation handed over" },
            ].map(({ period, label, desc }) => (
              <div key={period} className="flex flex-col gap-2">
                <span className="font-display font-bold text-xl text-brand-accent2">{period}</span>
                <span className="font-body text-sm font-medium text-brand-text">{label}</span>
                <span className="font-body text-xs text-brand-subtle leading-relaxed">{desc}</span>
              </div>
            ))}
          </div>
        </div>
